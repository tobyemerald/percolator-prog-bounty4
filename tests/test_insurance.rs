...
fn test_limited_insurance_withdraw_defaults_enforced() {
    ...
    assert_eq!(
        insurance_before, 10_000_000_000,
        "precondition: insurance seeded"
    );
...
fn test_limited_insurance_withdraw_custom_policy_enforced() {
    ...
    let delegated_set_attempt =
        env.try_set_insurance_withdraw_policy(&delegated, &delegated.pubkey(), 1, 10_000, 1);
    assert!(
    ...
    let delegated_mutation_attempt =
        env.try_set_insurance_withdraw_policy(&delegated, &delegated.pubkey(), 1, 10_000, 1);
    assert!(
...
fn test_limited_insurance_policy_validation_and_resolution_gates() {
    ...
    env.set_slot(12);
    let over_bps =
        env.try_set_insurance_withdraw_policy(&admin, &delegated.pubkey(), 1000, 10_001, 1);
...
fn test_limited_insurance_withdraw_adversarial_guards() {
    ...
    assert!(
        valid.is_ok(),
        "valid limited withdraw should succeed: {:?}",
        valid
    );
...
fn test_admin_withdraw_insurance_bypasses_limited_policy() {
    ...
    assert!(
        limited.is_ok(),
        "delegated 1% withdraw should succeed: {:?}",
        limited
    );
    assert_eq!(
        env.read_insurance_balance(),
        9_900_000_000,
        "insurance after limited withdraw"
    );
...
fn test_insurance_withdraw_cooldown_enforcement() {
    ...
    env.try_set_insurance_withdraw_policy(&admin, &admin.pubkey(), 1, 10_000, 1000)
        .unwrap();
...
fn test_insurance_withdraw_bps_cap() {
    ...
    env.try_set_insurance_withdraw_policy(&admin, &admin.pubkey(), 1, 5_000, 1)
        .unwrap();
...
fn test_deposit_cap_widened_unblocks_deposit() {
    ...
    env.svm
        .send_transaction(tx)
        .expect("init with live withdrawals");
...
fn test_insurance_withdraw_disabled_on_live_market() {
    ...
    assert_eq!(
        env.vault_balance(),
        vault_before,
        "vault_balance must be preserved after rejection"
    );
    assert_eq!(
        env.read_insurance_balance(),
        insurance_before,
        "insurance_balance must be preserved after rejection"
    );
}
...
fn test_update_authority_oracle_clears_price_when_no_policy_configured() {
    ...
    env.svm
        .airdrop(&new_oracle.pubkey(), 1_000_000_000)
        .unwrap();
    ...
    assert_eq!(
        price_after, 0,
        "authority_price_e6 cleared on rotation (no policy)"
    );
    assert_eq!(
        ts_after, 0,
        "authority_timestamp cleared on rotation (no policy)"
    );
}
...
fn send_withdraw_limited(env: &mut TestEnv, operator: &Keypair, amount: u64) -> Result<(), String> {
    ...
    let (vault_pda, _) =
        SdkPubkey::find_program_address(&[b"vault", env.slab.as_ref()], &env.program_id);
    let ix = Instruction {
        program_id: env.program_id,
        accounts: vec![
